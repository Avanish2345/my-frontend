...
function UploadPage() {
  ...
  // Fetch history from MongoDB Atlas on component mount
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/translations`); // Replace with your backend endpoint
        const data = await response.json();
        if (response.ok) {
          setHistory(data); // Set the fetched history
        } else {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedHistory = await fetch(`${import.meta.env.VITE_API_URL}/translations`);
        const updatedHistoryData = await updatedHistory.json();
        setHistory(updatedHistoryData); // Update the history
      } else {
        setTranslatedText(`Error: ${data.error || 'Unknown error'}`);
      }
    } catch (err) {
  ...
  const handleClearHistory = async () => {
    try {
      ...
      if (response.ok) {
        setHistory([]); // Clear history locally
      } else {
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            {history.length === 0 ? (
              ...
              <ul>
                {history.map((item, index) => (
                  <li key={index}>
                    <strong>Original Text:</strong> {item.sourceText}<br />
                    <strong>Translated Text:</strong> {item.translatedText}<br />
